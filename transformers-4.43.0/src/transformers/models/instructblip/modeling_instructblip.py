...
class InstructBlipForConditionalGeneration(InstructBlipPreTrainedModel):
    ...
    def generate(
        ...
        attention_mask: Optional[torch.LongTensor] = None,
        interpolate_pos_encoding: bool = False,
        use_damro: bool=False,
        damro_alpha: Optional[float]=None,
        damro_beta: Optional[float]=None,
        damro_topk: Optional[int] = None,
        **generate_kwargs,
    ) -> torch.LongTensor:
        """
        ...
        if use_damro:
            ...
            global outputs
            outputs = {}
            hook_handle_qkv = self.vision_model.encoder.layers[-2].self_attn.qkv.register_forward_hook(hook_qkv)
        ...
        if use_damro:
            ...
            desired_layer_qkv = outputs["desired_qkv"]
            hook_handle_qkv.remove()

            bsz, tgt_len, embed_dim = desired_layer_qkv.size()
 
     
            num_heads=self.vision_model.encoder.layers[-2].self_attn.num_heads
            embed_dim=self.vision_model.encoder.layers[-2].self_attn.embed_dim

            mixed_qkv = desired_layer_qkv.reshape(bsz, tgt_len, 3, num_heads, embed_dim // num_heads).permute(
                2, 0, 3, 1, 4
            )
            query_states, key_states, value_states = mixed_qkv[0], mixed_qkv[1], mixed_qkv[2]
            ...
            d= C // num_heads
            attn = (query_states @ key_states.transpose(-2, -1)) * d ** -0.5
            attn = F.softmax(attn, dim=-1)

            attn=attn.mean(dim=1) #[b,head,q_len,k_len]->[b,q_len,k_len]
            ...
            cls_attn = attn[:, 0, 1:]            
            _, idx = torch.topk(cls_attn, damro_topk, dim=1, largest=True)  # [B, left_tokens] , sorted=True
            index = idx.unsqueeze(-1).expand(-1, -1, C)  # [B, left_tokens, C]
            image_embeds_cd = torch.gather(image_embeds, dim=1, index=index)  # [B, left_tokens, C]
        ...
        if use_damro:
            ...
            query_output_cd = query_outputs_cd.last_hidden_state[:, : query_tokens.size(1), :]
