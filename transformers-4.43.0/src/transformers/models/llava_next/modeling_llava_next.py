...
class LlavaNextForConditionalGeneration(LlavaNextPreTrainedModel):
    ...
    def pack_image_features(self, image_features, image_sizes, image_newline=None):
        ...
        for image_idx, image_feature in enumerate(image_features):
            if image_feature.shape[0] > 1:
                ...
                # print(num_patch_height, num_patch_width,height, width)
    ...
    def pack_image_features_damro(self, image_features, image_sizes, image_newline=None):
        """
        image_features for damro
        """
        new_image_features = []
        feature_lens = []

        for image_idx, image_feature in enumerate(image_features):
            # image_feature: [N_selected, D]
            if image_feature.ndim == 3:
                # 有批维时取第一个（通常是 [1, N, D]）
                image_feature = image_feature[0]

            # 不再 reshape 2D grid，直接线性拼接
            if image_newline is not None:
                image_feature = torch.cat(
                    [image_feature, image_newline[None, :].to(image_feature.device, image_feature.dtype)],
                    dim=0,
                )
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1 and pixel_values.size(0) > 0:
                ...
                if damro_topk!=None:
                    ...
                    global outputs
                    outputs = {}
                    hook_handle_k = self.vision_tower.vision_model.encoder.layers[vision_feature_layer].self_attn.k_proj.register_forward_hook(hook_k)
                    hook_handle_q = self.vision_tower.vision_model.encoder.layers[vision_feature_layer].self_attn.q_proj.register_forward_hook(hook_q)
                ...
                if damro_topk!=None:
                    ...
                    B, N, C = selected_image_feature.shape
                    ...
                    selected_image_feature = torch.gather(selected_image_feature, dim=1, index=index)  # [B, left_tokens, C]
                ...
                if damro_topk!=None:
                    image_features, feature_lens = self.pack_image_features_damro(
                        image_features,
                        image_sizes,
