...
def eval_model(args):
    ...
    with open(answers_file, "w") as ans_file:
        for i in tqdm(range(0, len(questions), batch_size)):
            ...
            images = [Image.open(os.path.join(args.image_folder, b["image"])) for b in batch]
            prompts = [ b["text"] + " Please answer this question with one word." for b in batch]
            
            if "llava" in model.config.model_type.lower():
                prompts = ["<image>\n" + b["text"] + " Please answer this question with one word." for b in batch]
                conv_prompts = []
                for qs in prompts:
